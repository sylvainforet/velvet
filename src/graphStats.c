...
////////// Removes un-merged potential haploid sequences

static void
nodeKaboomBaby(Node *node,
	       Graph *graph,
	       ReadSet *reads,
	       boolean *dubious,
	       Coordinate minLength,
	       ...
	       char kaboomPass)
{
	...
	IDnum readID;
	IDnum index2;
	const boolean denounceReads = readStartsAreActivated(graph);

	if (kaboomPass == 1 && hasReferenceMarker(node, reads))
		return;
	if (kaboomPass == 2 && terminalReferenceMarker(node, reads))
		return;
	...
	if (denounceReads) {
		...
		maxIndex = getNodeReadCount(node, graph);
		for (index2 = 0; index2 < maxIndex; index2++) {
			shortMarker = getShortReadMarkerAtIndex(nodeArray, index2);
			readID = getShortReadMarkerID(shortMarker);

			if (readID > 0)
				dubious[readID - 1] = true;
			else
				dubious[-readID - 1] = true;
		}
	      ...
	      char kaboomPass)
{
	...
	if (getNodeLength(hapA) == 0
	    || getTotalCoverage(hapA) / (Time)getNodeLength(hapA) > maxHapCov
	    || getNodeLength(hapB) == 0
	    || getTotalCoverage(hapB) / (Time)getNodeLength(hapB) > maxHapCov)
		return;
	...
	if (getMultiplicity(arcA) + getMultiplicity(getArc(hapA)) < getMultiplicity(arcB) + getMultiplicity(getArc(hapB)))
		nodeKaboomBaby(hapA, graph, reads, dubious, minLength, outfile, kaboomPass);
	else
		nodeKaboomBaby(hapB, graph, reads, dubious, minLength, outfile, kaboomPass);
}

// TODO remove this code dupplication from correctedGraph.c
		      ...
		      Time maxDipCov)
{
	...
	if (getNodeLength(origin) == 0
	    || getTotalCoverage(origin) / (Time)getNodeLength(origin) > maxDipCov
	...
	for (arc = getArc(origin); arc != NULL; arc = getNextArc(arc)) {
		...
		if (getNodeLength(end) == 0
		    || getTotalCoverage(end) / (Time)getNodeLength(end) > maxDipCov)
			continue;
		  ...
		  char kaboomPass)
{
	...
	candidates = findHapLoopCandidates(origin, 3, maxHapCov, maxDipCov);

	while (candidates != NULL) {
		HapLoopCandidate *next;

		if (candidates->hapA != NULL && candidates->hapB != NULL)
			removeHapLoop(origin,
				      candidates->hapA,
				      candidates->hapB,
				      candidates->end,
				      graph,
		  ...
		  char kaboomPass)
{
	...
	if (dest == twin
	    ...
	    || getTotalCoverage(dest) / (Time)getNodeLength(dest) > maxDipCov)
		return;
...
void
removeUnmergedHaplotypes(Graph *graph,
			 Time maxHapCov,
			 Time maxDipCov,
			 ReadSet *reads,
			 boolean *dubious,
			 ...
			 char *filename)
{
	...
	velvetLog("Removing unmerged haplotypes !!!\n");
	...
	for (index = -nodes; index <= nodes; index++) {
		...
		removeHapLoopNode(node, graph, maxHapCov, maxDipCov, reads, dubious, minLength, outfile, 1);
		removeHapDeadEnd1(node, graph, maxHapCov, maxDipCov, reads, dubious, minLength, outfile, 1);
		removeHapDeadEnd2(node, graph, maxHapCov, maxDipCov, reads, dubious, minLength, outfile, 1);
	}
	...
	for (index = -nodes; index <= nodes; index++) {
		...
		removeHapLoopNode(node, graph, maxHapCov, maxDipCov, reads, dubious, minLength, outfile, 2);
		removeHapDeadEnd1(node, graph, maxHapCov, maxDipCov, reads, dubious, minLength, outfile, 2);
		removeHapDeadEnd2(node, graph, maxHapCov, maxDipCov, reads, dubious, minLength, outfile, 2);
	}
